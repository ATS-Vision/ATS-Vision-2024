 ...
 * 对 YOLOX 模型的输出进行后处理
 * 生成目标检测的提议框，
 * 并将相关信息保存在 objects 向量中,
 * 提议框的信息包括顶点坐标、矩形区域、类别、颜色
 * **/
static void generateYoloxProposals(std::vector<GridAndStride> grid_strides,
...
/**
 * 降序快速排序算法
 * 用于对存储在 faceobjects 向量中的 ArmorObject 结构体按照其 prob 字段进行降序排序
 * **/
...
/**
 *  objects 向量中的 ArmorObject 结构体按照其 prob 字段进行降序排序
 * **/
...
static void nms_sorted_bboxes(std::vector<ArmorObject>& faceobjects,
...
bool ArmorDetector::initModel(std::string path)
{
    ...
    ////// //Step 5. Start Inference 这部分可以debug
    // infer_request.start_async();
    // infer_request.wait();
